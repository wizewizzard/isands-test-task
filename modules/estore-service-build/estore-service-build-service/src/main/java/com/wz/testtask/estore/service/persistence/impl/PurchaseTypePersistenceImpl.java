...
public class PurchaseTypePersistenceImpl
	...
	public PurchaseType findByGroupIdName(long groupId, String name)
		...
		PurchaseType purchaseType = fetchByGroupIdName(groupId, name);
	...
	public PurchaseType fetchByGroupIdName(
		...
		if (result instanceof PurchaseType) {
			...
			if ((groupId != purchaseType.getGroupId()) ||
				!Objects.equals(name, purchaseType.getName())) {
				...
				if (list.isEmpty()) {
					...
					if (list.size() > 1) {
						...
						if (_log.isWarnEnabled()) {
							...
							_log.warn(
								"PurchaseTypePersistenceImpl.fetchByGroupIdName(long, String, boolean) with parameters (" +
	...
	public PurchaseType removeByGroupIdName(long groupId, String name)
		...
		PurchaseType purchaseType = findByGroupIdName(groupId, name);
	...
	public void cacheResult(PurchaseType purchaseType) {
		...
		finderCache.putResult(
			...
			new Object[] {purchaseType.getGroupId(), purchaseType.getName()},
			purchaseType);
	}
	...
	protected void cacheUniqueFindersCache(
		...
		args = new Object[] {
			purchaseTypeModelImpl.getGroupId(), purchaseTypeModelImpl.getName()
		};

		finderCache.putResult(
			_finderPathCountByGroupIdName, args, Long.valueOf(1), false);
		finderCache.putResult(
			_finderPathFetchByGroupIdName, args, purchaseTypeModelImpl, false);
	}
